...
function registerUserForSession(data) {
  ...
  return { success: true, message: `¡Registre completat per a ${targetSession.name}! Revisa el teu correu electrònic per a la confirmación.` };
}
...
function handleCancellationRequest(registrationId, token) {
  ...
  const nameCol = header.indexOf("UserName");
...
function sendConfirmationEmail(recipientEmail, recipientName, session, registrationId, cancellationToken) {
  ...
  const body = `
    ...
    Si no pots fer clic a l'enllaç, copia y engancha la següent URL al teu navegador:<br>
...
function getAllActiveRegistrations() {
    ...
    const regCol = {
        ...
        phone: header.indexOf("UserPhone"),
        email: header.indexOf("UserEmail"),
...
function sendDailyLists() {
    ...
    dailyRegistrations.forEach(reg => {
        ...
        emailBody += `
            <tr>
                ...
                <td>${reg.email}</td>
                <td>${reg.isMember}</td>
                <td>${reg.pago || ''}</td> <td>${reg.asistencia || ''}</td> </tr>
        `;
    });
    ...
    const subject = `Listado de Registros Activos - ${formattedDate}`;
    ...
    try {
        ...
        Logger.log(`INFO: Email de listado diario enviado a ${DAILY_LIST_RECIPIENT} para el ${formattedDate}.`);
    } catch (emailError) {
        Logger.log(`ERROR: Error al enviar el email de listado diario a ${DAILY_LIST_RECIPIENT}: ${emailError.message}`);
    }
}
...
function resetWeeklyRegistrations() {
    ...
    if (lastRow > 1) {
        registrationsSheet.deleteRows(2, lastRow - 1);
        Logger.log(`INFO: Se han borrado ${lastRow - 1} registros de la hoja '${REGISTRATIONS_SHEET_NAME}'.`);
