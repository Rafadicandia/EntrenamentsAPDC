...
function handleCancellationRequest(registrationId, token) {
  ...
  const nameCol = header.indexOf("User-Name"); // CAMBIADO: Asegúrate que coincide con el nombre de la columna en tu hoja
...
function sendDailyLists() {
    ...
    const excelHeaders = ["Hora Registro", "Nombre", "Teléfono", "Email", "Socio/a", "Pago", "Asistencia"];
    ...
    const fileName = `Listado_Registros_${formattedDate}.xlsx`;
    ...
    try {
        ...
        const emailBodyHtml = `
            ...
            <body>
                ...
                <p>Se adjunta el listado de registros activos en formato Excel.</p>
